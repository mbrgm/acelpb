...
let
  cfg = config.virtualisation.docker-test;
...

{
  ...
  options.virtualisation.docker-test = {
    enable =
      mkOption {
        type = types.bool;
        default = false;
    ...
    restartIfChanged =
      mkOption {
        ...
        description =
          ''
            This option blocks the docker daemon restarting automatically when
            the service is changed (e.g. after a system upgrade).
            Change to true to revert to restarting automatically.
          '';
      };
  };
